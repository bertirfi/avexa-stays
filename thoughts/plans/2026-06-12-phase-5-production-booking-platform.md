...
> Date: 2026-06-12 · Author: Opus 4.8 (orchestrator), interview with Robert same day
...
| D2 | **Pricing pipeline (§3.1):** Hostaway base RON x (1 + markup, global **18%**, via `AVEXA_MARKUP_PERCENT`) then / BNR daily rate x (1 + FX margin **3%**), rounded **up** to whole EUR/night = **effective EUR = what we charge = the "member rate".** |
| D3 | **Account required to book** (no guest checkout). Checkout keeps its auth gate. |
| D4 | **Member discount is presentation-only (§3.1), NOT a second subtraction.** Rack rate = effective / (1 - 0.15), struck-through, labelled "Member rate, 15% off". Logged-out guests see rack as the price; members see effective. base+markup already IS the member price — never subtract 15% from effective. |
...
`exchange_rates`; all RON→EUR conversion goes through one module; the full pricing pipeline (markup + FX + member presentation) is in §3.1.
...
1. **Base** — Hostaway live base price in RON (`listing.price`; for date-specific
   ...
   not its own markup fields.
2. **AVEXA markup** — ONE global percentage applied equally to all 8 apartments,
   configurable in ONE place via `AVEXA_MARKUP_PERCENT` (default **18**). Never
   per-listing, never hardcoded inline.
   `effective_RON = base_RON × (1 + markup)`
3. **RON → EUR** — at the BNR daily rate plus FX margin `AVEXA_FX_MARGIN_PERCENT`
   (default **3**), rounded **UP** to a whole euro per night:
   `effective_EUR = ceil( effective_RON ÷ BNR × (1 + fxMargin) )`
   This `effective_EUR` **IS what we charge** and **IS the "member rate".**
4. **Member discount = presentation only, NOT a second subtraction.**
   `rack_EUR = round( effective_EUR ÷ (1 − 0.15) )` shown struck-through next to the
   effective price, labelled "Member rate, 15% off". Logged-out guests see the rack
   rate AS the price; logged-in members see the effective price as their rate.
   base+markup ALREADY is the member price — never subtract 15% from effective.
...
## 3.2 Reservations & anti-double-booking (Wave 5)

After a successful Stripe payment, create the booking in Hostaway via
`POST /v1/reservations` (channel = direct/API, guest details, total paid). It then
shows in Hostaway as source "Direct/API", blocks the calendar, and auto-syncs to the
other channels. Strict order:

1. Re-check availability LIVE for the exact dates/units right before charging.
2. Charge Stripe.
3. Immediately create the Hostaway reservation(s).
4. If creation fails → auto-refund Stripe + alert ops (never leave a charge without a
   reservation).

Multi-room (D6): one Stripe charge for the order, then N reservations (one per suite),
grouped by `bookings.order_id`. We also store our own copy of every booking in Supabase
for My Trips / member history, but **Hostaway stays the source of truth for the
calendar.** Partial-failure policy = open item §10.
...
- **Wave 1 — Hostaway sync:** `lib/hostaway/client.ts` (OAuth2 `POST /v1/accessTokens`: grant_type=client_credentials, client_id=HOSTAWAY_ACCOUNT_ID, client_secret=HOSTAWAY_API_KEY, scope=general; token valid **24 months** -> store in Supabase service-role-only table, 1s wait after issuing, 403 -> refresh once and re-store; rate limits **15 req/10s per IP, 20 req/10s per account** -> cache hard, never call Hostaway live per visitor; wraps listings, calendar, priceDetails/Calculate-reservation-price, reviews; retries with backoff); `/api/sync/hostaway` + Vercel cron 15 min + manual trigger (SYNC_SECRET); adapter Hostaway→`Property` type; stays/locations pages read from Supabase (ISR 15 min). UI looks identical, data is real.
...
- **Wave 6 — SEO + analytics + polish:** canonicals everywhere; homepage title/description/H1 fix (primary keyword); OG/Twitter images; `not-found.tsx` + `error.tsx`; security headers; GA4 + PostHog EU + cookie consent; `/locations/[slug]` pages (4 × 800+ words — orchestrator drafts, Robert approves) with Google Maps Embed; slug 301s; sitemap update; icon-in-tile fix + design-drift cleanup (BRAND.md known-drift lists); video-ready hero slot.
...
App config (server-only, single source for the §3.1 pricing pipeline):
`AVEXA_MARKUP_PERCENT` (default 18), `AVEXA_FX_MARGIN_PERCENT` (default 3).
...
| Double-discount / wrong markup | Single `AVEXA_MARKUP_PERCENT`; member 15% is presentation-only; server recomputes from live base before every charge |
