...
- ✅ **P6b — REAL test + idempotency (2026-07-02): PASSED, cleanup pending.** Idempotency: Stripe Resend on `evt_1Tohd...` → 200, DB unchanged (1 event, 1 booking). Real test: mock removed → Little Gem Nov 17–19 ×2 → Stripe RON 588 (548+40) → **Hostaway reservation `62347427` (real)**, guest "Robert Ilie (TEST)", 588 RON, full comment; calendar Nov 17+18 `reserved` (~30s propagation), Nov 19 open (checkout day); our cache blocked instantly. **Finding: Hostaway ignores `isPaid:1` on POST** → `isPaid:null`, `paymentStatus:"Unknown"`, `status:"modified"` — needs follow-up fix (PUT after create or payment record). Cleanup: cancel 62347427 from dashboard → resync cache → mark test bookings cancelled in DB. Also pending: confirm Hostaway guest-confirmation email arrived (decision 13).
