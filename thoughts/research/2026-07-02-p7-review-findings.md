...
3. **F3 ✅ FĂCUT + VERIFICAT LIVE (2026-07-03):** my-trips randează bookings reale (RLS, view-model server-side, date fixe-locale fără `new Date(str)`); oglinda localStorage (`avexa_user`/`avexa_has_trips`/`avexa:auth-changed`) ștearsă complet — useAuth() + props server; verificat pe preview cu test-user-ul (Past stays + Cancelled badges reale, anonim → 307 /login).
4. **F4 ✅ FĂCUT + VERIFICAT LIVE:** `/api/quote` nou (schema Zod partajată cu checkout în `lib/booking/schema.ts`) — checkout-ul randează DOAR numerele serverului + banner de drift; assert `total_ron×100 === amount_total` în webhook; supersede pending-uri stale cu gardă anti-cursă (sesiune PLĂTITĂ în alt tab → 409 `already_processing`, nu anulare — găsit de mine la review, banii nu pot rămâne fără rezervare/refund); parseYmd/ymd (off-by-one TZ); poller cu fallback; autocomplete + Link. Verificat live: 560+0+40=600 RON identic cu formula, 401/400/409 corecte.
5. **F5 ✅ FĂCUT + VERIFICAT LIVE:** 429 retry mărginit + single-flight token; `lib/hostaway/sync.ts` = sursa unică de derivare cache (fallback listing.price identic cron/webhook), fereastră **380 zile** (sync live: toate 8 proprietățile, 381 zile fiecare); anularea eliberează cache-ul prin property row; **adopția rezervării orfane** înainte de refund (HostawayAmbiguousCreateError + findRecentDirectReservation pe canal 2000 + date + email); alertă ops la eșec charge; regex CA lărgit + deadline 230s în after().
6. **F6 ✅ FĂCUT + VERIFICAT LIVE:** reset-password blocat fără sesiune de recovery (PASSWORD_RECOVERY handled); `/api/newsletter` real (Zod + honeypot silent-drop + Brevo; 503 onest până apare BREVO_API_KEY în Vercel); profil: phone + marketing_consent prin server action Zod (RLS-scoped), toggle-ul fără coloană eliminat.
7. **F7 ✅ FĂCUT:** `lib/timing-safe.ts` pe toate rutele cu secret; diagnostic 404 în producție; robots disallow-all în afara producției; origin allowlist pe redirect-urile Stripe; safeNext parse pozitiv; payload Hostaway explicit; socials rel/target; force-dynamic member; pill factual; prefers-reduced-motion pe Editorial; propertyCount derivat live; cva scos + range-uri aliniate (next 15.5.20, tailwind 4.3.2 GA). CVE-ul postcss vendorizat de Next rămâne upstream-only — NU rula `npm audit fix --force`.

**Rămase (nefixate deliberat, noted):** MonthGrid triplicat + focus-traps pe modale + inline clamp() styles (LOW polish, churn mare); fade-ul de intrare din layout-ul stacked sub reduced-motion; rate limiting pe /api/checkout (infrastructură nouă — decizie separată); rotire SYNC_SECRET (pending din sesiunea veche); CVE postcss (upstream).
