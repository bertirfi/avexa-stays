...
### 2026-09-04/10 — Member benefits 04.09 + guest checkout + sticky Book now (PR #21, ÎN AȘTEPTAREA migrării 005)
- **Guest checkout (decizia Robert 04.09):** poarta „Member or guest?" pe `/checkout` fără sesiune; panoul „What you give up as a guest" înainte de Pay (liniile de anulare = `CANCELLATION_POLICY` verbatim, deci nu pot drifta față de T&C/FAQ/email). Rând `user_id NULL` + `rate_plan 'non_refundable'` + CHECK în DB. **Migrarea `005_guest_checkout.sql` se rulează manual ÎNAINTE de merge**, apoi `npm run db:types` (tipurile sunt editate de mână până atunci).
- **Adversarial review a prins 2 blockere reale înainte de producție:** (1) webhook-ul Stripe ar fi „adoptat" o rezervare Hostaway deja legată de alt booking → client taxat de două ori (două taburi / `Alice@` vs `alice@` / guest+membru) — acum verifică `hostaway_reservation_id` legat la alt rând înainte de adopție; (2) `/api/quote` public lovea Hostaway live per request, fără limită — un burst anonim ar fi blocat și webhook-ul de plată → quote-ul public citește cache-ul Supabase (`quoteBooking({source:'cache'})`), doar `/api/checkout` e live. Plus: sesiune lipsă fără flag `guest` = 401 (nu downgrade tăcut la non-refundable), supersede doar pentru membri, rate-limit per IP (`lib/rate-limit.ts`, per instanță — datorie: regulă Vercel WAF).
- **Sticky Book now** mărit ~176×55 (md) + halou alb; mobil `max-md:mb-24` sub How it works. Capturile de desktop din panoul Browser ascuns ies GOALE la viewport emulat — folosește gstack browse `screenshot --viewport`.
- **Minuta 04.09 (lot 10.09):** curățenia e inclusă în prețul afișat peste tot (checkout, sidebar, email, Stripe line item, carduri /locations cu date), împărțită egal pe nopți în lista per-noapte (`foldIntoNights`) — banii/Hostaway finance/extras jsonb rămân defalcați. Taxa de oraș afișată în RON real + „≈". **Bug Hostaway:** `cancelReservation` făcea DELETE (șterge rezervarea fără să elibereze calendarul — de-aia rămâneau datele blocate după „Cancel my trip"); corect e `PUT /reservations/{id}/statuses/cancelled` cu `cancelledBy`. Emailul de check-in cu linkul CA pleacă acum prin Brevo (office@), nu prin conversația Hostaway. Swipe în lightbox-ul galeriei (pointer events, touch-action pan-y).
- **Agent stall = limită de sesiune (429 „session limit")**, nu bug de cod — tree-ul rămâne curat sau parțial; relansează cu „păstrează ce există" în brief. Comenzile `browse js` cu `await fetch` întorc gol — pentru sonde API folosește `curl` din Bash.

### 2026-08-24/25 — Review Vlad 24.08 integral + readability + Brevo email (PR #16–#18)
- **Regula de cheltuire AVX = decizia Vlad 24.08, FINALĂ** (după 3 schimbări: Minuta inversată → Master v3 → 24.08): BRONZE–GOLD doar upsells deblocate 1 AVX=1 RON; PLATINUM+DIAMOND orice, cazare EXCLUSIV la 2 AVX=1 RON; upsells niciodată gratuite („FREE subject to availability" eliminat peste tot, inclusiv tile-ul din Vault și nota Diamond din Meter). Textele PLATINUM/DIAMOND din tabel = formulările verbatim ale lui Vlad.
- **Curățenie studio = 120 RON FINAL** (Vlad V1, după ping-pong 120→125→120). Telefon nou peste tot: **+40 721 347 642** (lib/contact.ts). Zoom mobil reactivat (era userScalable:false ca obiect JS în layout — grep pe stringul HTML nu-l prinde!).
- **Social: NICIUN cont AVEXA nu există** (Instagram/TikTok/Facebook @avexastays inexistente; linkedin.com/company/avexa = altă firmă) — scoase din footer; de recreat când Vlad face conturile. Newsletter ascuns până la double opt-in Brevo (NewsletterForm + /api/newsletter păstrate).
- **Email confirmare rezervare prin Brevo** (lib/email/brevo.ts `bookingConfirmationEmail` + `after()` în webhook Stripe; idempotent prin lock-ul existent pe booking; nu poate pica webhook-ul). Sender verificat în Brevo: DOAR office@avexastays.com (bookings@ ar fi respins — ⚠️ BREVO_SENDER_EMAIL din env trebuie să fie office@ sau lipsă). Template testat real prin Brevo API (send reușit 24.08). BREVO_API_KEY pus în Vercel 24.08 — env-urile noi cer REDEPLOY ca să intre în funcțiune.
- **Readability (cererea Robert):** body Manrope variabil, weight 400→450 site-wide (titluri neatinse); contrast WCAG măsurat matematic: white/35–45 pe ink = 3.2–4.5:1 (FAIL text mic) → white/55 (6.06:1) pe footnotes/footere carduri/eyebrows My Trips; token ink-60: 0.6→0.65 (4.47→5.2:1). Fix suprapunere icon pe cardurile featured MemberPerks (coloană fixă 64px + padding în celulă → `auto`).
- Dovezi V1 trimise lui Robert (capturi producție: curățenie 120/150/180 separată + taxa oraș linie proprie + defalcare per noapte). V2 cere o rezervare reală + 24h. V3 = fluxul site (factura NU e site — M5 scos din scope-ul Berty de Robert; la fel M11 „încă nu", CRM-ul separat).
- **Prețurile Hostaway au devenit DINAMICE** (variază 242–726 RON) — fereastra from-price 30 zile (aleasă 23.08) e acum semnificativă.
...
## Sesiunea 19.09.2026 — CRM preia emailurile de check-in; butonul „Add” la servicii
- **CRM (commit 046bdf7, producție) trimite singur emailul de confirmare + link de check-in** pentru rezervările directe (Brevo, office@, șablon în Check-in Admin). Site-ul NU mai trimite emailul cu link (`lib/hostaway/confirmation.ts` + `getCrmCheckinUrl` șterse); păstrăm bonul de rezervare (receipt: preț, extras, condiții de anulare — date pe care CRM-ul nu le are), refund/conflict, anulare, alertele interne. Cardul „Check-in & access” din My Trips se afișează doar când CRM-ul răspunde `reservation.direct === true` (OTA = chat-ul platformei).
- **URGENT până la go-live-ul CRM:** CRM-ul e în mod test — linkul de check-in arată „Invalid or expired link” oricui nu e pe lista de test. Cardul din My Trips va arăta linkul acela oaspeților reali de pe site până la go-live; decizia de a-l ascunde temporar e a lui Robert/Vlad.
- **Deliverability:** domeniul n-are SPF/DKIM Brevo/DMARC — recordurile exacte (Vercel DNS) sunt în INTEGRATIONS.md → Brevo; întâi se șterg recordurile Resend de pe `send`. Resend a dispărut complet din docs (CLAUDE.md, INTEGRATIONS.md, skill deploy).
- **UI extras:** o singură componentă `AddPill` / `AddPillBadge` (components/extras) pentru „+ Add” / „✓ Added” pe carduri, sidebar și My Trips; rândurile-buton poartă `group` ca badge-ul să reacționeze la hover. Review independent: `cn()` obligatoriu când baza și starea setează același utilitar (bg-white vs bg-gold-pale).
