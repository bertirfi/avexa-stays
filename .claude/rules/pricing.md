...
- Guests see ONE "total price" line for accommodation = accommodation + cleaning — never break out the markup, no "cleaning not included" wording; a "Cleaning fee" line appears ONLY inside the expandable per-night breakdown (client 11.09: guests see what they pay), never as a top-level line (client decision 04.09). City tax is its own line: real RON + "≈ equivalent" (never a rounded foreign amount). Extras are their own line. Cleaning stays its own line ONLY on ops surfaces (Hostaway finance lines, DB `extras`). "11% VAT included" — VAT is in the price, never added on top. No percentage discounts anywhere; cancellation rights come from membership (DX7: member 100% ≥72h / 50% 72–24h / 0% <24h; city tax always refunded), single rate — no saver/flex split.
