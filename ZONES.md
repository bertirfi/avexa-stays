...
- **Invarianți:** RON = money of record; preț = ceil(bază×1.21), NIMIC altceva; re-verificare live Hostaway înainte de Stripe; suma liniilor per-noapte = totalul încasat prin construcție; curățenie 120/150/180 încasată separat și AFIȘATĂ ca linie proprie „Cleaning fee” vizibilă în Price details / checkout, sub linia de cazare (client 12.09 — înlocuiește 04.09 „ascunsă” și 11.09 „doar în interiorul breakdown-ului”); prețurile de pe carduri includ curățenia; taxă oraș 10×nopți×pers pass-through, afișată în RON real + „≈”; DX7: 100/50/0 + taxa oraș integral înapoi. Regula completă: `.claude/rules/pricing.md`.
...
- **Stadiu:** ✅ live și dovedit (V1 cu capturi). ✅ Guest checkout (10.09): rând cu `user_id NULL` + `rate_plan 'non_refundable'` (CHECK în DB, migrarea 005), prețul re-derivat LIVE doar în `/api/checkout`; `/api/quote` (public) citește din cache-ul Supabase; webhook-ul Stripe nu mai „adoptă" o rezervare deja legată de alt booking (dublă plată → refund, nu dublă confirmare). ✅ Upsells (11.09): catalog AVX-08 în `lib/extras.ts` (UNICA sursă de prețuri), selectabile pe pagina apartamentului + sidebar, câte o linie „Extra · …” în breakdown și în Stripe, stocate în `bookings.extras` jsonb; add-on după rezervare din My Trips = sesiune Stripe separată (`startExtrasCheckout`, metadata kind='extras') aplicată de webhook cu idempotență pe sessionId, dedupe pe id, update optimist pe total_ron și refund automat dacă rezervarea nu mai e confirmată; la anulare refund-ul pe sejur se calculează FĂRĂ add-on-uri, iar add-on-urile se returnează integral doar dacă lead time-ul nu a trecut (AVX-08 §6). Lead time: extras expirate sunt ascunse în UI și eliminate în quote. Rămas: plata cu AVX la checkout/upsells (⏳ contabilă, CT2 TVA); Hostaway NU primește liniile de add-on (doar email ops pe office@).

## Z3 · Integrări externe
- **Cod:** `lib/hostaway/*` (server-only!), `app/api/webhooks/{stripe,hostaway}`, `lib/email/brevo.ts`, `lib/maps`, `lib/supabase/*`, `lib/analytics.ts` + `components/consent/AnalyticsGate.tsx` (PostHog)
- **Invarianți:** Hostaway DOAR server-side, disponibilitatea din cache-ul Supabase; semnături webhook verificate; email doar prin Brevo din office@avexastays.com (singurul sender verificat); env-urile noi din Vercel cer REDEPLOY. Reguli: `.claude/rules/hostaway.md`. PostHog: evenimente DOAR prin `track()` din `lib/analytics.ts`, niciodată nume/email/telefon/id-uri de rezervare sau Stripe; SDK-ul se încarcă doar după consimțământul Analytics; text de membru nou pe ecran → clasa `ph-mask`.
- **Modifici sigur:** orice endpoint nou → Zod + Bearer secret; niciun apel extern nou fără try/catch care nu poate pica webhook-ul.
- **Stadiu:** ✅ Hostaway sync + Stripe + Brevo confirmare rezervare (testat 24.08). 🔧 10.09: anularea în Hostaway folosea `DELETE /reservations/{id}` (șterge, NU eliberează calendarul) → acum `PUT /reservations/{id}/statuses/cancelled` (documentat „Cancel a reservation"); de verificat pe prima anulare reală că datele se eliberează singure. Emailul de check-in (link ChargeAutomation) pleacă prin Brevo din office@, nu prin conversația Hostaway (dublura raportată 04.09). 🔶 14.09 PostHog (EU, proiect 242490): cod complet — proxy `/lumen` în `middleware.ts` cu cookie-urile șterse (NICIODATĂ rewrites din next.config: trimiteau sesiunea Supabase la PostHog), consimțământ → încărcare/opt-out, identify pe user id Supabase, funnel search_submitted → booking_started → payment_started → booking_confirmed (+ anulare, newsletter, signup/login), erori din `app/error.tsx`, `?session_id` mascat în URL-uri și replay. Setările proiectului (replay, heatmaps, excepții, web vitals, IP anonimizat) erau deja pornite. Activare producție = `NEXT_PUBLIC_POSTHOG_KEY` în Vercel + redeploy (Cookie Policy v3.3 e deja pe site — vezi Z10). Rămas: webhook Hostaway consumers extinși, automatizări mesaje (M4.2, CRM-side).
...
- **Invarianți:** 1 h1/pagină, title <60, description <155, canonical; JSON-LD valid; keyword-ul principal pe home+locations; sub-2s mobil = regresiile sunt bug-uri; `images.qualities` allowlist; `skipTrailingSlashRedirect` e activ (cerut de proxy-ul PostHog) — redirect-ul canonic `/x/ → /x` (308) trăiește în `middleware.ts`, nu-l scoate.
...
- **Stadiu:** ✅ 11.09: documentele PUB v3.2 montate VERBATIM din Drive (`content/legal/*-v3-2.tsx` → /terms, /cancellation, /damage-policy, /house-rules, /imprint, /avexian-club-terms, /privacy, /cookies, /extra-services, /faq), fiecare cu „Document AVX-xx · Version 3.2 · In force from 1 September 2026” și URL permanent `/<slug>/v3-2` (noindex); ODR eliminat (Block A ziua 1); Block H făcut (AMEX scos, D3 wording, „Fast human support on WhatsApp”, vault = catalogul AVX-08). Regula: textul legal NU se reformulează niciodată pe site — se schimbă doar din documentele lui Vlad. Rămas: versiunile RO (după avocat, D4), log de consimțământ server-side (Block E), „Classification: application submitted” pe pagina unității (D7), moneda implicită RON (D11 — decizie); ✅ curs live BNR ziua anterioară +1% (`lib/fx-rates.ts`, cron `/api/cron/fx` pe `curs.bnr.ro`, EUR+USD, fallback env); ✅ cookie consent intern live (25.08): banner cu opțiuni egale, cookie unic `avexa_consent` 6 luni + Secure, revocabil din footer, /cookies = inventarul real; hărțile se afișează întotdeauna (decizia Robert 25.08 — risc ePrivacy rezidual asumat, de confruntat cu AVX-07). 🔶 14.09: categoria Analytics devine reală DOAR cu `NEXT_PUBLIC_POSTHOG_KEY` setat (toggle în setări, `CONSENT_VERSION` 2 → toți vizitatorii sunt întrebați din nou); fără cheie totul rămâne ca înainte („Not in use yet”). ✅ 14.09 **AVX-07 v3.3** (`content/legal/cookies-v3-3.tsx`, /cookies, arhivă v3.2 la /cookies/v3-2): §6 rescris pentru PostHog (cookie `ph_*` 6 luni + localStorage, ce se colectează, replay cu inputuri mascate, legat de cont prin ID intern, retenție 12 luni / 30 zile, EU, retragere = ștergere). EXCEPȚIE ASUMATĂ de Robert de la regula „verbatim din Drive”: textul a fost scris pe site întâi; ⏳ documentul AVX-07 din Drive trebuie adus la zi de Vlad ca să redevină sursa unică. Tema mare transversală: back-office pentru conținut (❌ — decizie de arhitectură înainte de M4).
