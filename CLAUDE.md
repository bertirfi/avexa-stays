...
- **Stripe** Checkout + webhooks · **Google Maps** (JS + Embed + Places) · **Brevo** for all email: auth (Supabase SMTP → Brevo), receipt / refund / cancellation notices, newsletter list. The CRM (AVEXA Automation) sends the check-in-link email for direct reservations
